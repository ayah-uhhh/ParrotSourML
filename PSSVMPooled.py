...
def svm_pool(minsize=10, maxsize=20):
    if __name__ == '__main__':
        ...
        for size in range(minsize, maxsize):
            results.extend([pool.apply_async(psvm.psSVM, args=([False, "linear", x, 'ovr', size, False, False]))
                            for x in (10**i for i in range(-2, 4))])
            results.extend([pool.apply_async(psvm.psSVM, args=([False, "rbf", x, 'ovr', size, False, False]))
                            for x in (10**i for i in range(-2, 4))])
